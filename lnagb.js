...
(function() {
  ...
  function isValidMatrix(matrix) {
    ...
    // Finally, check if every element is a number, let it be integer or float
    if (!matrix.every(function(row) {
  ...
  /**
   * Check if 2 given matrices are equal in dimensions.
   *
   * @param {object} matrixA A matrix
   * @param {object} matrixB Another matrix
   * @return {boolean} true if matrixA and matrixB have the same dimension,
   * false otherwise.
   */
  function equalDimension(matrixA, matrixB) {
    if (matrixA.length == matrixB.length && 
        getNumberOfElements(matrixA) == getNumberOfElements(matrixB)) {
      return true;
    }
    else {
      return false;
    ...
    if (equalDimension(matrixA, matrixB)) {
      ...
      for (var rowIndex = 0; rowIndex < numberOfRows; rowIndex++) {
        ...
        for (var columnIndex = 0; columnIndex < numberOfColumns; columnIndex++) {
          emptyRow.push(matrixA[rowIndex][columnIndex] + matrixB[rowIndex][columnIndex]);
        }
